...
const AnimalHealth: React.FC<AnimalHealthProps> = ({ onNavigate, veterinarians }) => {
  ...
  const mockProducts: Product[] = [
    { id: '1', name: 'Antibiótico 10mg', categoryId: 'cat-med', quantity: 50, salePrice: 45.00, isActive: true, createdBy: 'system', createdAt: new Date(), updatedAt: new Date() },
    { id: '2', name: 'Antipulgas Super-Cão', categoryId: 'cat-par', quantity: 100, salePrice: 60.00, isActive: true, createdBy: 'system', createdAt: new Date(), updatedAt: new Date() },
  ];

  const [appointments, setAppointments] = useState<Appointment[]>([
    { id: '1', animal: mockAnimals[0], animalId: '1', appointmentDate: new Date(2024, 11, 20), appointmentTime: '09:00', serviceType: mockServiceTypes[0], serviceTypeId: '1', veterinarian: veterinarians[0], veterinarianId: '1', status: 'scheduled', totalPrice: 80, isActive: true, createdBy: '1', createdAt: new Date(), updatedAt: new Date() }
  ]);
  ...
  const [dialogAction, setDialogAction] = useState<'edit' | 'complete' | 'cancel' | 'new' | null>(null);
  ...
  const handleOpenDialog = (appointment: Appointment | null, action: 'edit' | 'complete' | 'cancel' | 'new') => {
    if (action === 'new') {
        ...
        setAppointmentForm({
            ...
            services: [{ serviceTypeId: appointment.serviceTypeId, serviceType: appointment.serviceType, price: appointment.totalPrice }],
  ...
  const handleFinalizeAction = () => {
    ...
    if (editingAppointment) {
        ...
        if (dialogAction === 'complete') {
            ...
            handleSendToPOS(updatedAppointment);
  ...
  const handleSendToPOS = (appointment: Appointment) => {
    ...
    // Adiciona os serviços como itens de venda
    ...
    // Adiciona os produtos como itens de venda
    ...
    if(allItems.length === 0) {
        toast({ title: 'Nenhum item para cobrar', variant: 'destructive' });
        return;
    }

    onNavigate('point-of-sale', { draftSaleItems: allItems, customer: appointment.animal.tutor });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold text-gray-900">{t('clinic')}</h1><p className="text-gray-600">{t('manageAppointments')}</p></div>
        <Button className="bg-teal-600 hover:bg-teal-700" onClick={() => handleOpenDialog(null, 'new')}><Plus className="w-4 h-4 mr-2" />{t('newAppointment')}</Button>
      </div>
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div><CardTitle>{t('appointmentsList')}</CardTitle><CardDescription>{filteredAppointments.length} {t('appointmentsRegistered')}</CardDescription></div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="flex items-center space-x-2 mb-6">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="w-48"><SelectValue placeholder={t('allStatuses')} /></SelectTrigger><SelectContent><SelectItem value="all">{t('allStatuses')}</SelectItem><SelectItem value="scheduled">{t('scheduled')}</SelectItem><SelectItem value="completed">{t('completed')}</SelectItem><SelectItem value="cancelled">{t('cancelled')}</SelectItem></SelectContent></Select>
          </div>
          <Table>
            <TableHeader><TableRow><TableHead>{t('animal')}</TableHead><TableHead>Tutor</TableHead><TableHead>{t('service')}</TableHead><TableHead>{t('dateTime')}</TableHead><TableHead>{t('veterinarian')}</TableHead><TableHead>Valor Total</TableHead><TableHead>{t('status')}</TableHead><TableHead className="text-right">{t('actions')}</TableHead></TableRow></TableHeader>
            <TableBody>
              {filteredAppointments.map((appointment) => (
                <TableRow key={appointment.id}>
                  ...
                  <TableCell>R$ {(appointment.totalPrice).toFixed(2)}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end space-x-1">
                      ...
                      ) : ( <Button variant="ghost" size="icon" onClick={() => handleSendToPOS(appointment)} title="Ver no PDV"><Receipt className="w-4 h-4" /></Button> )}
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={(isOpen) => !isOpen && handleCloseDialog()}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><Label htmlFor="animal-form">{t('animal')}</Label><Select value={appointmentForm.animalId} onValueChange={(v) => setAppointmentForm(f => ({...f, animalId: v}))}><SelectTrigger id="animal-form"><SelectValue placeholder={t('selectAnimal')}/></SelectTrigger><SelectContent>{mockAnimals.map((a) => (<SelectItem key={a.id} value={a.id}>{a.name} - {a.tutor.name}</SelectItem>))}</SelectContent></Select></div>
              <div><Label htmlFor="vet-form">{t('veterinarian')}</Label><Select value={appointmentForm.veterinarianId} onValueChange={(v) => setAppointmentForm(f => ({...f, veterinarianId: v}))}><SelectTrigger id="vet-form"><SelectValue placeholder={t('selectVeterinarian')}/></SelectTrigger><SelectContent>{veterinarians.map((v) => (<SelectItem key={v.id} value={v.id}>{v.name}</SelectItem>))}</SelectContent></Select></div>
              <div><Label htmlFor="date-form">{t('date')}</Label><Input id="date-form" type="date" value={appointmentForm.appointmentDate} onChange={(e) => setAppointmentForm(f => ({...f, appointmentDate: e.target.value}))}/></div>
              <div><Label htmlFor="time-form">{t('time')}</Label><Input id="time-form" type="time" value={appointmentForm.appointmentTime} onChange={(e) => setAppointmentForm(f => ({...f, appointmentTime: e.target.value}))}/></div>
              <div><Label htmlFor="room-form">{t('room')}</Label><Select value={appointmentForm.roomId} onValueChange={(v) => setAppointmentForm(f => ({...f, roomId: v}))}><SelectTrigger id="room-form"><SelectValue placeholder={t('selectRoom')}/></SelectTrigger><SelectContent>{mockRooms.map((r) => (<SelectItem key={r.id} value={r.id}>{r.name}</SelectItem>))}</SelectContent></Select></div>
            </div>
      ...
      <Dialog open={isProductDialogOpen} onOpenChange={setIsProductDialogOpen}>
        <DialogContent>
          ...
          <div className="py-4"><Input placeholder={t('searchProduct')} value={productSearch} onChange={e => setProductSearch(e.target.value)}/>
